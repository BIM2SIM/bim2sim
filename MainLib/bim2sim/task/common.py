...
class LoadIFC(ITask):
    ...
    def get_ifc_structure(self, ifc):
        ...
        structure_decision = BoolDecision(question="Do you want to modify the elements.py based on the corresponding "
                                                   "schema of the ifc file? ",
                                          collect=False)
        structure_decision.decide()
        if structure_decision.value:
            # get schema version of ifc file
            self.get_elements_modifications()
            ...
            if len(links) == 1:
                ...
                self.logger.error('no corresponding schema found for the ifc file')
                return
