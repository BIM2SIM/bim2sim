...
class PlotResults(ITask):
    reads = ('df_finals', 'teaser_sim_results_path')
    final = True

    def run(self, df_finals, teaser_sim_results_path):
    ...
    def plot_total_consumption(self, df, teaser_sim_results_path, bldg_name):
        ...
        self.plot_time_series(
            ...
            data_time_series='heat_demand_total',
            data_total='heat_energy_total',
    ...
    def plot_time_series(
            ...
            df: pd.DataFrame,
            color,
            data_time_series: str,
            data_total:str = None,
            save_path: Optional[Path] = None,
            logo: bool = True, total_label: bool = True,
            fig_size: Tuple[int, int] = (10, 6),
            dpi: int = 300) -> None:
        save_path = save_path / f"{data_time_series}.pdf"
        # apply rolling mean for better visibility
        y_values = df[data_time_series].rolling(window=12).mean()
        if data_total:
            y_total = df[data_total].iloc[-1]
        ...
        if y_values.max() > 5000:
            ...
            plt.ylabel(f"heating Demand (kW)", labelpad=label_pad)
            y_values = y_values / 1000
        else:
            kw_labels = False
            plt.ylabel(f"heating Demand (W)", labelpad=label_pad)
        ...
        if total_label:
            plt.text(0.95, 0.95, f'Total energy: {y_total:.2f} kWh',
