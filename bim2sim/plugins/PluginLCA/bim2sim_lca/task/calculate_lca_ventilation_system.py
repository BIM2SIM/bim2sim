...
class CalculateEmissionVentilationSystem(ITask):
    ...
    def run(self, material_emission_dict, material_cost_dict):
        if self.playground.sim_settings.calculate_lca_ventilation_system:
            ...
            return (total_gwp_ventilation_duct,
                    total_gwp_ventilation_component,
                    total_cost_ventilation_duct,
                    total_cost_ventilation_component)
        else:
            self.logger.warning(
                f"Skipping task {self.name} as sim_setting "
                f"'calculate_lca_ventilation_system' "
                f"is set to "
                f"{self.playground.sim_settings.calculate_lca_ventilation_system}"
                f"and no LCA calculation of ventilation system should be"
                f" performed.")
