...
import pandas as pd
import colebrook
from bim2sim.utilities.visualize_graph_functions import \
    visualzation_networkx_3D, visulize_networkx
...
class CalcHeatingQuantities(ITask):
    ...
    def __init__(self, playground):
        ...
        self.specific_heat_capacity = 4.186 * (
    ...
    def run(self, heating_graph: nx.Graph()):
        ...
        for temperature in self.playground.sim_settings.design_distribution_temperatures:
            ...
            caluclated_heating_graph = self.number_delivery_nodes_each_room(
                heating_graph,
                component_type=self.playground.sim_settings.distribution_system_type)
            caluclated_heating_graph.graph[
                "temperature_niveau"] = temperature.value
            ...
            caluclated_heating_graph = self.initialize_node_physical_attributes(
                caluclated_heating_graph)
            # Add physical attributes to edges
            caluclated_heating_graph = self.initialize_edge_physical_attributes(
                caluclated_heating_graph)
            caluclated_heating_graph = self.initialize_node_parameters(
                caluclated_heating_graph,
                parameter_key="coefficient_resistance")

            # initialize or load parameter
            variable_zone_dict = self.load_simulation_results(
                self.playground.sim_settings.simulation_file_path)
            thermalzone_mapping = self.load_thermalzone_mapping(
                self.playground.sim_settings.thermalzone_mapping_file_path)
            ...
            caluclated_heating_graph = self.initialize_delivery_nodes(
                graph=caluclated_heating_graph,
            ...
            caluclated_heating_graph = self.calculate_distribution_pipe_system(
                caluclated_heating_graph)
            caluclated_heating_graph = self.calculate_edge_attributes(
                caluclated_heating_graph)

            # pressure difference pipes
            caluclated_heating_graph = self.iterate_pressure_loss_edges(
                caluclated_heating_graph)
            # pressure difference components
            caluclated_heating_graph = self.iterate_pressure_loss_fittings_nodes(
                caluclated_heating_graph)
            caluclated_heating_graph = self.calculate_pressure_loss_system(
                caluclated_heating_graph)
            min_pressure, max_pressure, bottleneck_node, pressure_difference, caluclated_heating_graph = self.calculate_network_bottleneck(
                caluclated_heating_graph, nodes=["delivery_node_supply"])
            # calculate pump power
            caluclated_heating_graph = self.calculate_pump_power(
                caluclated_heating_graph, pressure_difference)
    ...
    def calculate_pump_power(self,
        ...
        for pump in pump_node:
            ...
            graph.nodes[pump][operation_point]['power'] = pump_power.to(
                ureg.kilowatt)
    ...
    def calculate_pump_performance(self,
                                   ...
                                   efficiency):
        return round(
            (m_flow * pressure_difference) / (efficiency * self.density_water),
            2)
    ...
    def calculate_head(self, pressure_difference: float):
        return round((pressure_difference * 1 / (
                    self.density_water * self.g)).to_base_units(), 4)
    ...
    def calculate_pressure_loss_system(self,
        ...
        for iteration in range(max_iterations):
            prev_node_pressures = {
                node: graph.nodes[node][viewpoint]['pressure_out'] for node in
                graph.nodes}
            for node in list(nx.topological_sort(graph)):
                if "pump" == graph.nodes[node]["component_type"]:
                    prev_pressure_out = graph.nodes[node][viewpoint][
                        'pressure_out']
                    graph.nodes[node][viewpoint][
                        'pressure_out'] = prev_pressure_out
                    ...
                    for succ in successors:
                        ...
                        if len(predecessors) > 1:
                            ...
                            for pre in predecessors:
                                next_pressure_out = \
                                graph.nodes[pre][viewpoint]['pressure_out']
                                if next_pressure_out < min_pressure:
                                    min_pressure = next_pressure_out
                                    pre_node = pre
                            prev_pressure_out = \
                            graph.nodes[pre_node][viewpoint]['pressure_out']
                            edge_pressure_loss = \
                            graph[pre_node][next_node][viewpoint][
                                'pressure_loss']
                            node_pressure_loss = \
                            graph.nodes[pre_node][viewpoint]['pressure_loss']
                            next_pressure_in = prev_pressure_out - edge_pressure_loss
                            next_pressure_out = next_pressure_in - node_pressure_loss
                            graph.nodes[next_node][viewpoint][
                                'pressure_in'] = next_pressure_in
                            graph.nodes[next_node][viewpoint][
                                'pressure_out'] = next_pressure_out
                        else:
                            edge_pressure_loss = \
                            graph[node][next_node][viewpoint]['pressure_loss']
                            node_pressure_loss = \
                            graph.nodes[next_node][viewpoint]['pressure_loss']
                            next_pressure_in = prev_pressure_out - edge_pressure_loss
                            next_pressure_out = next_pressure_in - node_pressure_loss
                            graph.nodes[next_node][viewpoint][
                                'pressure_in'] = next_pressure_in
            ...
            for node in graph.nodes:
                pressure_diff = abs(
                    graph.nodes[node][viewpoint]['pressure_out'] -
                    prev_node_pressures[node])
    ...
    def load_pipe_data(self,
        ...
        for index, row in data.iterrows():
            try:
                ...
                wall_thickness = row[
                                     'normal_wall_thickness [mm]'] * ureg.millimeter
                material = row['material']
                pipe_mass = row['pipe_mass [kg/m]']
                density = row['density [kg/m³]'] * (
                            ureg.kilograms / ureg.meter ** 3)
        ...
        if len(inner_diameter_list) > 0:
            inner_diameter = min(inner_diameter_list,
                                 key=lambda x: abs(inner_diameter_list[
                                                       x] - calc_inner_diameter))
    ...
    def calculate_pipe_inner_diameter(self,
        ...
        return (((4 * mass_flow / (
                    math.pi * self.density_water * self.maximal_velocity)) ** 0.5) * security_factor).to(
            ureg.millimeter)
    ...
    def calculate_distribution_pipe_system(self,
        ...
        supply_line_graph = self.iterate_in_reversed_topological_direction(
            supply_line_graph,
            operation_point=viewpoint)
        # return line:
        return_line_graph = graph.subgraph(sorten_nodes["return_line"])
        return_line_graph = self.iterate_in_topological_direction(
            return_line_graph,
            viewpoint=viewpoint)
        # heating_graph
    ...
    def iterate_in_topological_direction(self,
        ...
        for node in list(nx.topological_sort(graph)):
            ...
            if len(predecessors) > 1:
                mass_flow_sum = sum(
                    graph.nodes[succ][viewpoint]['mass_flow'] for succ in
                    predecessors)
                volume_flow_sum = sum(
                    graph.nodes[succ][viewpoint]['volume_flow'] for succ in
                    predecessors)
                heat_flow_sum = sum(
                    graph.nodes[succ][viewpoint]['heat_flow'] for succ in
                    predecessors)
                ...
                graph.nodes[node][viewpoint]["mass_flow"] = \
                graph.nodes[predecessors[0]][viewpoint]['mass_flow']
                graph.nodes[node][viewpoint]["volume_flow"] = \
                graph.nodes[predecessors[0]][viewpoint]['volume_flow']
                graph.nodes[node][viewpoint]["heat_flow"] = \
                graph.nodes[predecessors[0]][viewpoint]['heat_flow']
        return graph
    ...
    def iterate_in_reversed_topological_direction(self,
        ...
        for node in reversed(list(nx.topological_sort(graph))):
            ...
            if len(successors) > 1:
                mass_flow_sum = sum(
                    graph.nodes[succ][operation_point]['mass_flow'] for succ in
                    successors)
                volume_flow_sum = sum(
                    graph.nodes[succ][operation_point]['volume_flow'] for succ
                    in successors)
                heat_flow_sum = sum(
                    graph.nodes[succ][operation_point]['heat_flow'] for succ in
                    successors)
                graph.nodes[node][operation_point]["mass_flow"] = mass_flow_sum
                graph.nodes[node][operation_point][
                    "volume_flow"] = volume_flow_sum
                graph.nodes[node][operation_point]["heat_flow"] = heat_flow_sum
            elif len(successors) == 1:
                graph.nodes[node][operation_point]["mass_flow"] = \
                graph.nodes[successors[0]][operation_point]['mass_flow']
                graph.nodes[node][operation_point]["volume_flow"] = \
                graph.nodes[successors[0]][operation_point]['volume_flow']
                graph.nodes[node][operation_point]["heat_flow"] = \
                graph.nodes[successors[0]][operation_point]['heat_flow']
    ...
    def initialize_delivery_nodes(self,
        ...
        """
        radiator_dict = self.load_radiator_model(
            filename=self.playground.sim_settings.distribution_file_path,
            sheet_name=self.radiator_sheet_name
            )
        for node, data in graph.nodes(data=True):
            if data["component_type"] == heating_system_type:
                for th in th_mapping:
                    if set(data["belongs_to_room"]) & set(
                            th_mapping[th]["space_guids"]):
                        # PHeater
                        heat_flow = th_mapping[th]["PHeater"]

                        data[operation_point]["heat_flow"] = (heat_flow / data[
                            "delivery_number"]) * ureg.kilowatt
                        data[operation_point][
                            "mass_flow"] = self.calculate_mass_flow(
                            heat_flow=data[operation_point]["heat_flow"],
                            supply_temperature=supply_temperature,
                            return_temperature=return_temperature)
                        data[operation_point]["heat_flow_per_square_metre"] = (
                                    heat_flow / (
                                        data["room_area"] * ureg.meter ** 2))
                        data[operation_point][
                            "volume_flow"] = self.calculate_volume_flow(
                            data[operation_point]["mass_flow"])

                        # Norm indoor temperature
                        room_type = th_mapping[th]["usage"]
                        temp_unit = \
                        self.room_parameter["standard_indoor_temperature"][
                            "unit"]
                        try:
                            temperature = \
                            self.room_parameter["standard_indoor_temperature"][
                                "room"][room_type]
                            data[operation_point][
                                "norm_indoor_temperature"] = temperature * temp_unit
                        except KeyError:
                            data[operation_point][
                                "norm_indoor_temperature"] = 293.15 * temp_unit
                        # radiator
                        if heating_system_type == "radiator":
                            log_op_temp = self.calculate_logarithmic_mean_temperature(
                                standard_indoor_temperature=
                                data[operation_point][
                                    "norm_indoor_temperature"],
                                supply_temperature=supply_temperature,
                                return_temperature=return_temperature)
                            heat_flow_norm = self.calculate_norm_heat_flow_radiator(
                                heat_flow_operation=data[operation_point][
                                    "heat_flow"],
                                logarithmic_mean_temperature_operation=log_op_temp)
                            mass_flow_norm = self.calculate_mass_flow(
                                heat_flow=heat_flow_norm,
                                supply_temperature=supply_temperature,
                                return_temperature=return_temperature)
                            volume_flow_norm = self.calculate_volume_flow(
                                mass_flow_norm)
                            selected_model, min_mass, material, length, norm_heat_flow = self.calculate_radiator_model(
                            ...
                            graph.nodes[node][
                                'norm_heat_flow_per_length'] = norm_heat_flow
                        if heating_system_type == "underfloor_heating":
                            ...
                            heating_medium_overtemperature = self.calculate_logarithmic_mean_temperature(
                                standard_indoor_temperature=
                                data[operation_point][
                                    "norm_indoor_temperature"],
                                supply_temperature=supply_temperature,
                            ...
                            thermal_resistance = 0.15 * ((
                                                                     ureg.meter ** 2 * ureg.kelvin) / ureg.watt)

                            underfloor_heating_pipe_length = self.calculate_underfloor_heating_pipe_length(
                                room_area=data["room_area"] * ureg.meter ** 2,
                                laying_distance=0.2)
                            graph.nodes[node][
                                'underfloor_heating_pipe_length'] = underfloor_heating_pipe_length
                            graph.nodes[node][
                                'underfloor_heating_material'] = self.calculate_mass_pipe_material(
                                density=960 * (
    ...
    def load_radiator_model(self,
        ...
        for index, row in data.iterrows():
            ...
            if not pd.isnull(row['Normwärmeleistung ((75/65/20 °C) in W/m']):
                data_dict["Normwärmeleistung"] = row[
                                                     'Normwärmeleistung ((75/65/20 °C) in W/m'] * (
                                                         ureg.watt / ureg.meter)
            if not pd.isnull(row['Wasserinhalt in l/m']):
                data_dict["Wasserinhalt"] = row['Wasserinhalt in l/m'] * (
                            ureg.liter / ureg.meter)
            if not pd.isnull(row['Masse in kg/m']):
                data_dict["Masse"] = row['Masse in kg/m'] * (
    ...
    def calculate_radiator_model(self,
        ...
        for model in radiator_dict:
            if 'Wasserinhalt' and 'Masse' and 'Normwärmeleistung' and 'Material' in \
                    radiator_dict[model]:
                volume_per_length = radiator_dict[model]['Wasserinhalt']
                mass_per_length = radiator_dict[model]['Masse']

                norm_heat_flow_per_length = radiator_dict[model][
                    'Normwärmeleistung']
    ...
    def calculate_logarithmic_mean_temperature(self,
        ...
        return (supply_temperature - return_temperature) / \
            (math.log((supply_temperature - standard_indoor_temperature) / (
                        return_temperature - standard_indoor_temperature)))
    ...
    def calculate_mass_flow(self,
        ...
        Returns:
            mass_flow ():
        """
        return (heat_flow / (self.specific_heat_capacity * (
                    supply_temperature - return_temperature))).to(
            ureg.kilogram / ureg.second)
    ...
    def load_simulation_results(self,
        ...
        """
                ...
                """
        self.logger.info(
            f"Load simulation result from file {simulation_file}.")
        ...
        for variable_name in variable_names:  # multizonePostProcessing.PHeaterSum
            if not "PHeater" in variable_name:
                continue
            ...
            if variable.find("[") > -1:
                ...
                result_dict = {time_column[i]: value_list[i][0] * 10 ** (-3)
    ...
    def initialize_node_parameters(self,
        ...
        for node, data in graph.nodes(data=True):
            try:
                value = self.components_parameter[parameter_key]["component"][
                    data["component_type"]]
                unit = self.components_parameter[parameter_key]["unit"]
                graph.nodes[node][operation_point][
                    parameter_key] = value * unit
            except KeyError:
                unit = self.components_parameter[parameter_key]["unit"]
                self.logger.warning(
                    f"Component {data['component_type']} not found. Set {parameter_key} value to {filling_parameter}.")
                graph.nodes[node][operation_point][
                    parameter_key] = filling_parameter * unit
    ...
    def calculate_edge_attributes(self,
        ...
        # iterative edges
        self.logger.info(
            f"Load pipe data with material {self.playground.sim_settings.distribution_pipe_material}.")
        for node, data in graph.nodes(data=True):
            successors = list(graph.successors(node))
            for succ in successors:
                mass_flow_out = graph.nodes[succ][operation_point]["mass_flow"]
                volume_flow_out = graph.nodes[succ][operation_point][
                    "volume_flow"]
                heat_flow_out = graph.nodes[succ][operation_point]["heat_flow"]
                graph.edges[node, succ][operation_point][
                    "mass_flow"] = mass_flow_out
                graph.edges[node, succ][operation_point][
                    "volume_flow"] = volume_flow_out
                graph.edges[node, succ][operation_point][
                    "heat_flow"] = heat_flow_out
                calc_inner_diameter = self.calculate_pipe_inner_diameter(
                    mass_flow=mass_flow_out)
                graph.edges[node, succ][operation_point][
                    "calculate_inner_diameter"] = calc_inner_diameter
                inner_diameter, outer_diameter, material, density = self.load_pipe_data(
                    filename=self.playground.sim_settings.distribution_file_path,
                ...
                graph.edges[node, succ][operation_point][
                    "material_mass"] = self.calculate_mass_pipe_material(
                    density=density,
                    length=graph.edges[node, succ]['length'],
                    inner_diameter=inner_diameter,
                    outer_diameter=outer_diameter)
                graph.edges[node, succ][operation_point][
                    "pipe_insulation"] = self.calculate_pipe_insulation(
                    outer_diameter)
        return graph

    def iterate_pressure_loss_edges(self, graph: nx.DiGraph(),
                                    operation_point: str = "design_operation_point"):
        for node in graph.nodes():
            successors = list(graph.successors(node))
            for succ in successors:
                length = graph.edges[node, succ][operation_point]['length']
                inner_diameter = graph.edges[node, succ][operation_point][
                    'inner_diameter']
                delta_p_friction, pipe_friction_resistance = self.calculate_friction_pressure_loss(
                    inner_diameter=inner_diameter,
                    v_mid=self.maximal_velocity / 2,
                    length=length)
                graph.edges[node, succ][
                    'pipe_friction_resistance'] = pipe_friction_resistance
                graph.edges[node, succ][operation_point][
                    'pressure_loss'] = delta_p_friction
        return graph
    ...
    def calculate_friction_pressure_loss(self, inner_diameter, v_mid, length):
        ...
        if reynold <= 2300:
            pipe_friction_coefficient = self.pipe_friction_coefficient_laminar_hagen_poiseuille(
                reynold=reynold)
        else:
            pipe_friction_coefficient = colebrook.sjFriction(reynold,
                                                             self.absolute_roughness)
    ...
    def calc_pipe_friction_resistance(self,
        ...
        pipe_friction_resistance = 0.5 * (
                pipe_friction_coefficient * self.density_water * (
                    1 / inner_diameter) * v_mid ** 2).to(
            ureg.Pa / ureg.m)
    ...
    def iterate_pressure_loss_fittings_nodes(self,
                                             ...
                                             operation_point: str = "design_operation_point") -> nx.DiGraph():
        for node, data in graph.nodes(data=True):
            coefficient_resistance = data[operation_point][
                "coefficient_resistance"]
            ...
            graph.nodes[node][operation_point][
                "pressure_loss"] = pressure_loss_node
        return graph

    def calculate_pressure_loss_fittings(self, coefficient_resistance: float,
                                         mid_velocity: float):
        """
        ...
        """
        return (
                    0.5 * self.density_water * coefficient_resistance * mid_velocity ** 2).to(
            ureg.pascal)
    ...
    def calculate_mass_pipe_material(self,
        ...
        return density * length * math.pi * (
                    outer_diameter ** 2 - inner_diameter ** 2) / 4
    ...
    def initialize_edge_physical_attributes(self,
        ...
        for edge in graph.edges():
            ...
            # variables
            graph.edges[edge][operation_point][
                "pressure_loss"] = 0.0 * 10 ** 5 * ureg.pascal
            graph.edges[edge][operation_point][
                "inner_diameter"] = 0.0 * ureg.meter
            graph.edges[edge][operation_point][
                "calculate_inner_diameter"] = 0.0 * ureg.meter
            graph.edges[edge][operation_point][
                "outer_diameter"] = 0.0 * ureg.meter
            graph.edges[edge][operation_point][
                "heat_flow"] = 0.0 * ureg.kilowatt
            graph.edges[edge][operation_point]["mass_flow"] = 0.0 * (
                        ureg.kilogram / ureg.seconds)
            graph.edges[edge][operation_point]["volume_flow"] = 0.0 * (
                        ureg.meter ** 3 / ureg.seconds)
            graph.edges[edge][operation_point][
                "pipe_friction_resistance"] = 0.0 * (ureg.Pa / ureg.m)
            graph.edges[edge][operation_point][
                "pipe_insulation"] = 0.0 * ureg.meter
            ...
            graph.edges[edge][operation_point][
                "maximal_velocity"] = self.maximal_velocity * (
                        ureg.meter / ureg.seconds)
            graph.edges[edge][operation_point]["length"] = graph.edges[edge][
                                                               "length"] * ureg.meter
        return graph
    ...
    def initialize_node_physical_attributes(self,
        ...
        for node, data in graph.nodes(data=True):
            ...
            # variables
            graph.nodes[node][operation_point][
                "max_velocity"] = self.maximal_velocity
            graph.nodes[node][operation_point]["mass_flow"] = 0.0 * (
                        ureg.kilogram / ureg.second)
            graph.nodes[node][operation_point][
                "heat_flow"] = 0.0 * ureg.kilowatt
            graph.nodes[node][operation_point]["volume_flow"] = 0.0 * (
                        ureg.meter ** 3 / ureg.second)
            graph.nodes[node][operation_point][
                "pressure_in"] = 1.0 * 10 ** 5 * ureg.pascal
            graph.nodes[node][operation_point][
                "pressure_out"] = 1.0 * 10 ** 5 * ureg.pascal
            graph.nodes[node][operation_point][
                "pressure_loss"] = 0.0 * 10 ** 5 * ureg.pascal
            graph.nodes[node][operation_point][
                "heat_flow_per_square_metre"] = 0.0 * (
                        ureg.kilowatt / ureg.meter ** 2)
            # parameter
            graph.nodes[node][operation_point]["head"] = 0.0
            graph.nodes[node][operation_point]["norm_indoor_temperature"] = (
                                                                                        22 + 273.15) * ureg.kelvin
            graph.nodes[node][operation_point][
                "coefficient_resistance"] = 0.0 * ureg.dimensionless
        return graph
            ...
            if component_type == data["component_type"]:
                for room_id in delivery_nodes_counter:
                    if room_id in data["belongs_to_room"]:
                        data["delivery_number"] = delivery_nodes_counter[
                            room_id]
        return graph
