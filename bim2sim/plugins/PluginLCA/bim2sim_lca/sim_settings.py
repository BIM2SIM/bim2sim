...
from bim2sim.sim_settings import BuildingSimSettings, BooleanSetting, ChoiceSetting, PathSetting, NumberSetting
...
class LCAExportSettings(BuildingSimSettings):
    ...
    update_emission_parameter_from_oekobdauat = BooleanSetting(
        default=False,
        description='Whether to update material emission parameter from '
                    'Ökobaudat',
        for_frontend=True
    )
    calculate_lca_building = ChoiceSetting(
        default=LCACalculationBuilding.granular,
        choices={
            LCACalculationBuilding.granular: "Take into account all materials"
                                             " and match them to ÖKUBAUDAT",
            LCACalculationBuilding.only_insulation: "Just take into account "
                                                    "insulation"
        },
    ...
    hydraulic_components_data_file_radiator_sheet = ChoiceSetting(
        default="Profilierte Flachheizkörper",
        choices={
            "Profilierte Flachheizkörper": "Profilierte Flachheizkörper"
        },
