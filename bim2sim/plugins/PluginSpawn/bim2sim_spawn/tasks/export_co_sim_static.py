...
class ExportModelicaSpawnStatic(ITask):
    ...
    def run(self, elements: dict):
        ...
        data = template.render(
            model_name=self.to_modelica('building_simulation'),
            model_comment=self.to_modelica('test2'),
            weather_path_ep=self.to_modelica(weather_path_ep),
            weather_path_mos=self.to_modelica(weather_path_mos),
            zone_names=self.to_modelica(zone_names),
            idf_path=self.to_modelica(idf_path)
        )
    ...
    def to_modelica(parameter):
        ...
        if isinstance(parameter, (list, tuple, set)):
            return "{%s}" % (
                ",".join((ExportModelicaSpawnStatic.to_modelica(par) for par in parameter)))
